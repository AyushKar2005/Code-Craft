...
function SnippetCard({snippet}:{snippet:Snippet}) {
    const {user} = useUser()
    const deleteSnippets = useMutation(api.snippets.deleteSnippet)
    const[isDeleting,setIsDeleting] = useState(false)

    const handleDelete = async()=>{
        setIsDeleting(true);
        try{
            await deleteSnippets({snippetId:snippet._id});
  ...
  return (
    <motion.div layout className="group relative" whileHover={{y:-1}} transition={{duration:0.2}}>
